...
class ForgotPasswordUsecase @Inject constructor(
    ...
    suspend fun forgotPassword(forgotPasswordReqDTO: ForgotPasswordReqDTO): Flow<Resource<BaseResponse?>> = flow {
        try {
            emit(Resource.Loading())
            val response = authRepository.forgotPassword(forgotPasswordReqDTO)
